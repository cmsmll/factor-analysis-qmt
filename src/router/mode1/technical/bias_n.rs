...
pub async fn router() -> Router {
    ...
    Router::new().push(
        Router::with_path(Req::id())
            .post(bias_n)
            .push(Router::with_path("detail").post(bias_n_detail)),
    )
}
...
/// 多日乖离率因子单日明细请求：因子参数 + 可选目标日期（缺省取筛选区间末交易日）。
#[derive(Debug, Serialize, Deserialize, ToSchema, validator::Validate)]
...
/// 开始喂 SMA，保证目标日的均线与主分析口径一致。
#[endpoint]
pub async fn bias_n_detail(args: VJson<DetailReq>) -> Resp<Arc<RawValue>> {
    let key = args.0.hashcode();
    match MODE1.details.get_or_run(key, move || bias_n_detail_run(args.0)).recv().await {
        Ok(res) => resolve!(res => 200, "ok"),
        Err(_) => reject!(400, "获取数据失败"),
    }
}
...
/// 计算目标日单日分位明细：预热 = `period` 个交易日，推进 SMA 后仅收集目标日当天的乖离率。
fn bias_n_detail_run(args: DetailReq) -> Box<RawValue> {
    ...
    for index in df.index_iter() {
        ...
        for (item, store) in df.list.iter().zip(store.iter_mut()) {
            ...
            {
                if is_target {
                    rows.push(DetailRow::new(&item.metadata, curr, finance, bias_factor(curr.close, avg_close), profit));
                }
            }
        }
    }
