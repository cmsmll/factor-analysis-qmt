...
pub async fn router() -> Router {
    ...
    Router::new().push(
        Router::with_path(Req::id())
            .post(volume_n)
            .push(Router::with_path("detail").post(volume_n_detail)),
    )
}
...
/// 多日均量因子单日明细请求：因子参数 + 可选目标日期（缺省取筛选区间末交易日）。
#[derive(Debug, Serialize, Deserialize, ToSchema, validator::Validate)]
...
/// 保证目标日的均量与主分析口径一致（SMA 只依赖最近 `period` 个交易日）。
#[endpoint]
pub async fn volume_n_detail(args: VJson<DetailReq>) -> Resp<Arc<RawValue>> {
    let key = args.0.hashcode();
    match MODE1.details.get_or_run(key, move || volume_n_detail_run(args.0)).recv().await {
        Ok(res) => resolve!(res => 200, "ok"),
        Err(_) => reject!(400, "获取数据失败"),
    }
}
...
fn volume_n_detail_run(args: DetailReq) -> Box<RawValue> {
